...
| `T1/T2/T3` | Temperature sensors (`OFF` — not connected; `33.5` — °C). After instrument cold start T1 is invalid for ≥4 s; the gateway does not use field `t`. T1 from the first USB-session `-inf` is withheld (JSON `null`); one follow-up `-inf` ~5 s later. `OFF` in JSON means no sensor, not 0 °C. |
